...
export class LoginComponent {
  ...
  login() {
    if (this.form.value.email && this.form.value.password) {
      ...
      this.http.post(`${AppConstants.BASE_URL_API}/login`, JSON.stringify(this.form.value), {headers}).subscribe(
        {
          next: (body: any) => {
            if (body?.accessToken && body?.tokenType) {
              this.sessionStorageService.setItem("access_token", body?.accessToken);
              this.sessionStorageService.setItem("token_type", body?.tokenType);
